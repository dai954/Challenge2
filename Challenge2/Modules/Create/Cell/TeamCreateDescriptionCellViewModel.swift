...
class TeamCreateDescriptionCellViewModel: ViewModelType {
    let id = UUID().uuidString
    
    struct Input {
        let textEditingEvent: Driver<String>
    }
    ...
    func transform(input: Input) -> Output {
        ...
        let validatedTextField = input.textEditingEvent
            .skip(2)
            .map { text -> TextFieldValidationResult in
                if (1...400) ~= text.count {
                    return .ok(message: "\(text.count)/400文字")
                } else if text.count > 400 {
                    return .over(message: "\(text.count)/400文字")
                } else {
                    return .empty
                }
            }
